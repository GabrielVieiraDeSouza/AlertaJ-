...
async function CriarModalGraficoTemperatura(CidadeId,NomeCidade) {
    ...
    const Resultado = await fetch('http://localhost:3000/clima');
...
async function PesquisarCidade(Texto) {
    CidadesAtivas = []
    const Resultado = await fetch("http://localhost:3000/cidades")
    const Dados = await Resultado.json()
...
async function CriarCard(Cidade) {
    ...
    const ResultadoCidades = await fetch("http://localhost:3000/cidades")
