...
- Usuários finais — moradores de áreas de risco, famílias e agricultores (consomem alertas e informações).
...
- Orientação em situações de emergência (histórico, clima e notícias por localidade);
- Apoio à tomada de decisão em momentos críticos.

Transformações entregues: informação confusa passa a alertas claros; falta de tempo passa a avisos antecipados; insegurança passa a mais confiança para agir.
...
A partir da Homepage, o usuário acessa as seções de consulta (Histórico por Local, Notícias, MiniHub de Temperaturas) e a Sidebar de Configurações. Pela Sidebar, ele pode entrar/cadastrar-se (Login), gerenciar o perfil (Meus Dados) e ajustar tema, notificações e região principal. O Cadastro de Alertas de Risco é uma área restrita, acessível apenas a administradores.

```
Homepage
  |-- Histórico por Local
  |-- Notícias  -->  Detalhe da matéria
  |-- MiniHub de Temperaturas
  |-- Configurações (Sidebar)
  |     |-- Entrar / Cadastrar  -->  Login  -->  sessão
  |     |-- Meus Dados (Perfil: editar / excluir)
  |     |-- Tema, Notificações, Região Principal
  |-- Cadastro de Alertas (somente administradores)
```

A Sidebar de Configurações é acessível de qualquer tela (link "Configurações" ou ícone de engrenagem) e concentra login, perfil e preferências. O acesso ao Cadastro de Alertas é protegido: apenas administradores entram.

### Wireframes

O grupo produziu wireframes de baixa fidelidade das telas principais (home, perfil/"Meus Dados", histórico e cadastro de alertas), que serviram de base para a construção das interfaces finais.

### Protótipo Interativo

A própria aplicação, executada localmente via Live Server, funciona como protótipo navegável da solução: é possível percorrer todas as telas e funcionalidades como no software pronto (ver a seção 5. Solução Implementada).
...
| Backend simulado | json-server | Cria uma API REST completa a partir de um arquivo db.json, sem back-end real. |
...
| Gestão de tarefas | Trello / GitHub Projects | Organização das tarefas no quadro Kanban. |
...
- Gabriel Vieira atuou também como integrador, responsável pela montagem da homepage e pela junção dos módulos no GitHub;
...
O acompanhamento das tarefas foi feito em um quadro Kanban, organizado nas colunas "A fazer", "Em andamento" e "Concluído". Cada tarefa correspondeu a um módulo ou ajuste da aplicação, permitindo visualizar o progresso ao longo das três Sprints.
...
- Acesso e uso: na Sidebar (Configurações), clicar em "Entrar" para login com e-mail e senha, ou "Cadastre-se" para criar conta (nasce como usuário comum). Contas de teste: administrador admin@alertaja.com / admin123; comum user@alertaja.com / user123.

### Sidebar de Configurações e Tema Claro/Escuro (Ferlânio)
- Descrição: painel lateral, presente em todas as telas, com tema claro/escuro global, notificações de alerta e seleção de região principal. O tema é aplicado a toda a aplicação e persiste após recarregar a página.
- Estrutura de dados associada: coleção configuracoesUsuario.
- Acesso e uso: clicar em "Configurações" (ou no ícone de engrenagem) e usar os interruptores e a seleção de região.

### Meus Dados / Perfil (Ferlânio)
- Descrição: gerenciamento do perfil do usuário logado — visualizar, editar e excluir a conta (operações Read, Update e Delete do CRUD).
- Estrutura de dados associada: coleção usuarios.
- Acesso e uso: Sidebar, opção "Meus Dados".

### Cadastro de Alertas de Risco (Rafael)
- Descrição: CRUD completo de alertas preventivos (título, tipo, cidade, bairro, nível de risco, descrição e data). Restrito a administradores.
- Estrutura de dados associada: coleção alertas.
- Acesso e uso: logado como administrador, acessar a página de Cadastro de Alertas, preencher o formulário e clicar em "Cadastrar"; editar ou excluir pela lista.
...
### MiniHub de Consulta de Temperaturas (Gabriel)
- Descrição: consulta de temperatura, umidade e vento por cidade (dados reais da OpenWeatherMap) e gráfico do histórico de 30 dias, desenhado em um elemento canvas.
...
- Descrição: exibição das notícias e alertas recentes em cards (imagem, categoria, título e descrição), com página de detalhe da matéria.
...
- Bootstrap 5.3 — grid responsivo, carrossel, modal e componentes de interface;
- Google Fonts (Montserrat, Poppins) — tipografia;
- Canvas API — gráficos de temperatura e umidade do MiniHub;
...
APIs utilizadas para acesso a dados e serviços:
...
- OpenWeatherMap API — dados reais de clima (temperatura e umidade) e geocodificação das cidades.

### Como executar o projeto

1. Instalar as dependências (uma vez): na pasta src, rodar `npm install` (o json-server já vem no repositório).
2. Iniciar a API: na pasta src, rodar `npx json-server db.json` (porta 3000).
3. Abrir a aplicação: com o Live Server (VS Code), clicar com o botão direito em src/index.html e escolher "Open with Live Server".
...
- GOTHELF, J.; SEIDEN, J. Lean UX. (Referência sobre Design Thinking, personas e histórias de usuário.)
